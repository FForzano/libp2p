...
class pktTracker:
    ...
    def check_pkt(self, pktid):
        ...
        if pktid in self.pkts.keys():
            return False
        
        self.pkts[pktid] = int(time.time())
        return True
    ...
    def _refresh(self):
        ...
        for pkt in self.pkts.keys():
            if (current_time - self.pkts[pkt]) < self.listen_time:
                temp_pkts[pkt] = self.pkts[pkt]
